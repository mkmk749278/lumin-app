...
class OrderExecutor {
  ...
  /// Entry-only market order taken from a MARKET ALERT (Pulse → Alerts).
  ///
  /// Explicit owner decision: an alert is an observation, not a signal —
  /// it has no SL/TP geometry, so "Take trade" places ONLY the market
  /// entry.  No stop and no take-profit are placed; the confirm sheet
  /// carries the unmissable warning and the user manages the exit
  /// themselves (Binance or the Trade tab).  The engine never manages
  /// this position; it is a manual trade on the user's own account,
  /// same custody class as Take Signal.
  ///
  /// Idempotency mirrors ``placeFromSignal``: the alert_id keys the
  /// order log AND the broker ``clientOrderId``.
  Future<ExecutionResult> placeAlertEntry({
    required int userId,
    required String alertId,
    required String symbol,
    required String direction, // LONG / SHORT
    required BinanceKeys keys,
    required AutoTradeSettings settings,
    required double equity,
    required double markPrice,
  }) async {
    final logKey = 'alert-$alertId';
    final existing = await _logService.entryFor(userId, logKey);
    if (existing != null) {
      return ExecutionResult(
        success: true,
        alreadyTaken: existing,
    ...
    if (markPrice <= 0) {
      return _fail('No live price for $symbol — try again.');
    }
    final leverageHardCapped = leverageReq.clamp(1.0, 30.0).toInt();
    ...
    final client = _clientFactory(keys);
    try {
      final filters = await client.getSymbolFilters(symbol);
      final notional = equity * (sizingPct / 100.0) * leverageHardCapped;
      final rawQty = notional / markPrice;
      final qty = filters.roundQty(rawQty);
      if (qty <= 0.0) {
        return _fail(
          'Size too small: ${rawQty.toStringAsFixed(6)} $symbol rounds '
          'below minQty (${filters.minQty}). Increase position size.',
        );
      }
      if (qty * markPrice < filters.minNotional) {
        return _fail(
          'Notional too small: ${(qty * markPrice).toStringAsFixed(2)} USDT '
          '< minNotional ${filters.minNotional} USDT. Increase position size.',
        );
      }

      await client.setLeverage(symbol, leverageHardCapped);

      final entryOrder = await client.createMarketOrder(
        symbol: symbol,
        side: side,
        quantity: qty,
        clientOrderId: 'lumin-entry-$logKey',
      );
      final entryId = (entryOrder['orderId'] as num?)?.toInt();
      final avgPrice = _readDouble(entryOrder['avgPrice']);
      ...
      final entry = OrderLogEntry(
        ...
        testnet: keys.testnet,
        avgFillPrice: avgPrice > 0 ? avgPrice : null,
        executionMode: 'alert-manual',
        entryPriceTarget: markPrice,
      );
      await _logService.record(userId, entry);
      ...
      return ExecutionResult(
        success: true,
        entry: entry,
        message: 'Entry placed: $qty $symbol '
            '@ ${avgPrice > 0 ? "\$${avgPrice.toStringAsFixed(4)}" : "market"}. '
            'No SL/TP — manage this position yourself.',
      );
    } on BinanceError catch (e) {
      return _fail('${e.message} (code ${e.code ?? "?"})');
    } catch (e) {
